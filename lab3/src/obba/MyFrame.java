...
public class MyFrame extends JFrame {
	...
	public MyFrame(Double[] coefficients) {
		...
		helpMenuItem.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent ev) {
				JOptionPane.showMessageDialog(MyFrame.this, "student");
			}
		});
		...
		Action saveToCSVAction = new AbstractAction(
		...
		Action searchValueAction = new AbstractAction(
			...
			public void actionPerformed(ActionEvent event) {
				String value = JOptionPane.showInputDialog(MyFrame.this,
		...
		Action searchIntervalAction = new AbstractAction(
				"Найти значения в интервале многочлена") {
			public void actionPerformed(ActionEvent event) {
				String value = JOptionPane.showInputDialog(MyFrame.this,
						"Введите start значение для поиска", "Поиск значения",
						JOptionPane.QUESTION_MESSAGE);
				renderer.setFrom(value);
				value = JOptionPane.showInputDialog(MyFrame.this,
						"Введите posl значение для поиска", "Поиск значения",
						JOptionPane.QUESTION_MESSAGE);
				renderer.setTo(value);
				getContentPane().repaint();
			}
		};
		searchIntervalMenuItem = tableMenu.add(searchIntervalAction);
		...
		hboxRange.add(Box.createHorizontalGlue());
		hboxRange.setPreferredSize(new Dimension(new Double(hboxRange
				.getMaximumSize().getWidth()).intValue(), new Double(hboxRange
				.getMinimumSize().getHeight()).intValue() * 2));
		...
		hboxButtons.setPreferredSize(new Dimension(new Double(hboxButtons
				.getMaximumSize().getWidth()).intValue(), new Double(
				hboxButtons.getMinimumSize().getHeight()).intValue() * 2));
	...
	public static void main(String[] args) {
		if (args.length == 0) {
			System.out
					.println("Невозможно табулировать многочлен, для которого не задано ни одного коэффициента!");
			System.exit(-1);
		}
