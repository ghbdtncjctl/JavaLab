...
public class GetAds extends SimpleTagSupport {
	...
	public void doTag() throws JspException, IOException {
		...
		if (id > 0) {
			...
			final User authUser = (User) getJspContext().getAttribute(
					"authUser", PageContext.SESSION_SCOPE);
			...
			for (Ad ad : adList.getAds()) {
				...
				if (!"my".equals(range)
						|| (authUser != null && authUser.getId() == ad
								.getAuthorId())) {
			...
			Comparator<Ad> comparator = new Comparator<Ad>() {
				public int compare(Ad ad1, Ad ad2) {
					int result;
					// Если выбрана сортировка по дате последнего изменения
					// объявления
					if (GetAds.this.sort != null
							&& GetAds.this.sort.equals("date")) {
						// То результат определяется по значениям поля
						// lastModified
						result = ad1.getLastModified() < ad2.getLastModified() ? -1
								: (ad1.getLastModified() > ad2
										.getLastModified() ? 1 : 0);
						// Если порядок сортировки обратный - инвертируем
						// результат сравнения
					...
					if (GetAds.this.sort != null
						...
						// Иначе сортируем по автору объявления по значениям
						// поля name автора
						result = ad1.getAuthor().getName().compareTo(
								ad2.getAuthor().getName());
